...
| P2-4 | **Trade + Onramp (1-day MVP, next-chapter slide)** | New tabs next to Cashout, FreeCash-style single-screen UI (big number + one button), NOT part of the Stocklana main path (Earn → $5 → Cashout stays primary). Ship as a working minimal add-on + one pitch slide/demo cut showing Earn→Own→Buy→Trade, not a full trading product. Scope: **(1) Trade** — Jupiter Swap API (quote → swap) for USDC ⇄ xStock SPL, starting with 1–3 symbols (e.g. AAPLx); reuse the existing Privy Solana embedded wallet (no new wallet flow); one screen: symbol / Buy-or-Sell / amount / Swap; monetize via Jupiter's `platformFeeBps` (~50–100 bps) + a referral account — explicitly OUT: order book, charts, leverage. **(2) Onramp** — a single embedded widget (MoonPay / Transak / Coinbase Onramp) targeting the user's existing Privy Solana address, asset = USDC; UI: "Add cash" → widget → redirect to Trade on completion; explicitly OUT: building our own card-payment or KYC flow. Suggested sequencing: AM = Trade MVP (even 1 symbol is fine), PM = Onramp widget, evening = pitch slide with the Earn→Own→Buy→Trade diagram + one demo cut. |
